...
Usage::
    ...
    python -m scripts.run_study_i_codebert
    python -m scripts.run_study_i_codebert --data-path data/bug_patterns_categorized.json
    python -m scripts.run_study_i_codebert --figures-dir figures --tables-dir tables
    python -m scripts.run_study_i_codebert --models roberta codebert
    python -m scripts.run_study_i_codebert --quick  # smoke-test

Outputs are split across two folders:

* ``--figures-dir`` (default ``./figures``):
    * ``fig1_confusion_matrix_<short>.{svg,png}``
    * ``fig2_fold_distribution_<short>.{svg,png}``
    * ``fig3_roc_curve_<short>.{svg,png}``
    * ``fig_cross_model_comparison.{svg,png}``
    * ``fig_roc_overlay.{svg,png}``
    * ``fig_paired_per_fold.{svg,png}``
...
# Make the project root importable when this script is run as a file.
...
from src.study_i.schemas import (  # noqa: E402
    MODEL_REGISTRY,
    TrainingConfig,
    get_default_config,
)
from src.study_i.dataset import (  # noqa: E402
...
from src.study_i.training import run_full_experiment  # noqa: E402
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--data-path", default=None,
        help="Path to the labeled bug-pattern JSON. Falls back to "
             "$DATA_PATH or data/bug_patterns_labeled.json.")
    parser.add_argument("--output-dir", default=None,
        help="Trainer scratch directory. Defaults to $OUTPUT_DIR or ./results.")
    parser.add_argument("--figures-dir", default=None,
        help="Where to write figures. Defaults to $FIGURES_DIR or ./figures.")
    parser.add_argument("--tables-dir", default=None,
        help="Where to write CSV/JSON tables. Defaults to $TABLES_DIR or ./tables.")
    parser.add_argument("--figure-format", default=None,
        choices=["svg", "png", "pdf"],
        help="Figure output format. Defaults to $FIGURE_FORMAT or 'svg'.")
    parser.add_argument("--models", nargs="*", default=None,
        choices=[m["short"] for m in MODEL_REGISTRY],
        help="Subset of backbones to run. Defaults to all three.")
    parser.add_argument("--quick", action="store_true",
        help="Smoke-test: 2 folds, 1 seed, 2 epochs.")
    return parser.parse_args(argv)


def select_registry(short_names: Optional[List[str]]):
    """Filter ``MODEL_REGISTRY`` down to ``short_names`` (preserving order)."""
    if not short_names:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.figures_dir:   config.figures_dir = args.figures_dir
    if args.tables_dir:    config.tables_dir = args.tables_dir
    if args.figure_format: config.figure_format = args.figure_format
    if args.quick:
        config.n_folds = 2
        config.cv_seeds = (42,)
        config.num_epochs = 2
    ...
    print(f"CV protocol: {config.n_folds}-fold x {len(config.cv_seeds)} "
          f"seeds = {config.total_folds_per_model} folds per model")
    print(f"Total fold-runs: "
    ...
    for entry in registry:
        summary = run_full_experiment(
            ...
            config=config, n_labeled=len(labeled),
        )
    ...
    if len(summaries) >= 2:
        ...
        print("\nCross-model comparison (mean +/- std across CV folds):\n")
    ...
    save_cross_model_results(
        ...
        cv_setup=(f"{config.n_folds}-fold x {len(config.cv_seeds)} "
                  f"seeds = {config.total_folds_per_model} folds per model"),
