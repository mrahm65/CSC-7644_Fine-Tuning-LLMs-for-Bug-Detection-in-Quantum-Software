"""Configuration schema and label space for Study I.
...
* ``FIGURES_DIR``  - Directory where SVG / PNG figures are written.
* ``TABLES_DIR``   - Directory where CSV / JSON result tables are written.
* ``FIGURE_FORMAT`` - One of ``"svg"`` (default), ``"png"``, ``"pdf"``.
...
def get_default_config() -> TrainingConfig:
    """Build a :class:`TrainingConfig` honoring all I/O env variables.
    ...
    return TrainingConfig(
        ...
        figure_format=os.environ.get("FIGURE_FORMAT", DEFAULT_FIGURE_FORMAT),
    )
