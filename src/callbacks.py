...
class ManualEarlyStoppingCallback(TrainerCallback):
    ...
    keep ``save_strategy='no'`` to avoid filling disk during cross-validation,
    so we re-implement just the patience logic here.
    """
...
class EpochLogCallback(TrainerCallback):
    ...
    Use this to render learning curves outside the trainer.
    """
...
def oversample_to_balance(
    ...
    Args:
        texts: Parallel list of training strings.
        labels: Integer labels aligned with ``texts``.
        rng: :class:`random.Random` instance (seeded by the caller).

    Returns:
        Shuffled ``(texts, labels)`` lists where every class has the same
        size as the largest class.
    """
    ...
    # Shuffle both lists together so batches are not class-blocked.
