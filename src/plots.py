...
def plot_confusion_matrix(
    ...
    *,
    ...
    out_path = os.path.join(
        output_dir, f"fig1_confusion_matrix_{model_short}.png"
    )
...
def plot_fold_distribution(
    ...
    *,
    ...
    for ax, vals, label, color in panels:
        ...
        ax.set_ylabel(label)
        ax.set_title(
            f"{label} across {len(vals)} folds", fontweight="bold"
        )
    ...
    plt.tight_layout()
    out_path = os.path.join(
        output_dir, f"fig2_fold_distribution_{model_short}.png"
    )
    plt.savefig(out_path, bbox_inches="tight")
    plt.close(fig)
    return out_path
...
def plot_roc_curve(
    ...
    *,
    ...
    for fold in results_all:
        fpr, tpr, _ = roc_curve(
            fold["y_true"], np.array(fold["probs"])[:, 1]
        )
    ...
    ax.set_title(
        ...
        f"Mean AUC = {pooled_auc:.3f} "
        f"(pooled across {len(results_all)} folds)",
        fontsize=12,
    )
    ...
    plt.tight_layout()
    out_path = os.path.join(
        output_dir, f"fig3_roc_curve_{model_short}.png"
    )
    plt.savefig(out_path, bbox_inches="tight")
    plt.close(fig)
    return out_path
...
def plot_cross_model_comparison(
    ...
    *,
    ...
    Also prints paired t-test outputs for adjacent and end-to-end pairs.
    """
    ...
    for ax, (mkey, skey, fkey, label, color) in zip(axes, metric_keys):
        ...
        # Paired t-tests across the matched fold-runs.
        pairs = [(0, 1), (1, 2), (0, 2)] if n_models >= 3 else []
        for a, b in pairs:
            try:
                t_stat, p_val = ttest_rel(
                    summaries[a][fkey], summaries[b][fkey]
                )
                if p_val < 0.001:
                    sig = "***"
                elif p_val < 0.01:
    ...
    plt.tight_layout()
    out_path = os.path.join(output_dir, "fig_cross_model_comparison.png")
    plt.savefig(out_path, bbox_inches="tight")
    plt.close(fig)
    return out_path
...
def plot_roc_overlay(
    ...
    *,
    ...
    ax.set_aspect("equal")
    plt.tight_layout()
    out_path = os.path.join(output_dir, "fig_roc_overlay.png")
    plt.savefig(out_path, bbox_inches="tight")
    plt.close(fig)
    return out_path
...
def plot_paired_per_fold(
    ...
    *,
    ...
    plt.tight_layout()
    out_path = os.path.join(output_dir, "fig_paired_per_fold.png")
    plt.savefig(out_path, bbox_inches="tight")
    plt.close(fig)
    return out_path
